...
class AuthRepository @Inject constructor(
    ...
    /** Emits the current user every time Firebase auth state changes (login/logout/token refresh). */
    ...
    suspend fun signUp(email: String, password: String): Result<FirebaseUser> = runCatching {
        ...
        result.user ?: error("Sign-up failed: no user returned")
    }

    suspend fun signIn(email: String, password: String): Result<FirebaseUser> = runCatching {
        val result = auth.signInWithEmailAndPassword(email, password).await()
        result.user ?: error("Sign-in failed: no user returned")
    }
